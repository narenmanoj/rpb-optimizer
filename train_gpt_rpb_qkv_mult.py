...
class RPB(torch.optim.Optimizer):
    ...
    def __init__(self, params, lr=0.5, momentum=0.95, nesterov=True, h_sigma=8.0,
                 ...
                 q_mult=1.0, k_mult=1.0, v_mult=1.0):
        ...
        # Optional per-block scaling for the Q/K/V activation-space target.
        # Baseline RPB uses (1, 1, 1); this lets us test whether equal
        # radius for score paths (Q,K) and value path (V) is too restrictive.
    ...
    def step(self):
        ...
        for group in self.param_groups:
            ...
            for p in group["params"]:
                ...
                scale_q = (self.q_mult * rstar).repeat_interleave(d_h)  # [d]
                scale_k = (self.k_mult * rstar).repeat_interleave(d_h)  # [d]
                scale_v = (self.v_mult * rstar).repeat_interleave(d_h)  # [d]
                scale = torch.cat([scale_q, scale_k, scale_v])          # [3d]
        ...
        self.last_diag = {
            ...
            "rpb/q_mult": self.q_mult,
            "rpb/k_mult": self.k_mult,
            "rpb/v_mult": self.v_mult,
        }
...
class Hyperparameters:
    ...
    # Optional per-block multipliers for Q/K/V activation-space target.
    rpb_q_mult : float = 1.0
    rpb_k_mult : float = 1.0
    rpb_v_mult : float = 1.0
...
args.rpb_q_mult = _env_f("RPB_Q_MULT", args.rpb_q_mult)
args.rpb_k_mult = _env_f("RPB_K_MULT", args.rpb_k_mult)
args.rpb_v_mult = _env_f("RPB_V_MULT", args.rpb_v_mult)
...
optimizer3 = RPB(qkv_weights, lr=args.rpb_eta, momentum=args.rpb_momentum,
                 ...
                 q_mult=args.rpb_q_mult, k_mult=args.rpb_k_mult, v_mult=args.rpb_v_mult)
