...
class _QKVCapture(torch.autograd.Function):
    ...
    """
    @staticmethod
    def forward(ctx, x2d: Tensor, weight: Tensor, ref: dict, capture: bool, rowsign_power: float):
...
class CausalSelfAttention(nn.Module):
    def __init__(self, config):
        ...
        if self.qkv_opt_mode not in {"hybrid", "newton_muon", "muon"}:
...
if args.qkv_opt_mode not in {"hybrid", "newton_muon", "muon"}:
    raise ValueError(
        f"QKV_OPT_MODE must be hybrid, newton_muon, or muon; got {args.qkv_opt_mode!r}"
    )
...
#   optimizer3 -> QKV hybrid RPB or exact Muon/Newton-Muon control
...
# Diagnostics: the exact controls have ordinary QKV gradients; the hybrid QKV
# parameters are driven through captured activation-space statistics.
named_params = {
