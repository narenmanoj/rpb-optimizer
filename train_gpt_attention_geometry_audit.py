...
class CausalSelfAttention(nn.Module):
    def __init__(self, config):
        ...
        if self.qkv_opt_mode not in {"hybrid", "bridge", "newton_muon", "muon"}:
    ...
    def forward(self, x, precond_flag: bool = False):
        ...
        if self.qkv_opt_mode in {"hybrid", "bridge"}:
            ...
            if precond_flag and self.qkv_opt_mode == "newton_muon":
...
class Hyperparameters:
    ...
    qkv_opt_mode : str = "hybrid"  # hybrid, newton_muon, or muon
...
if args.qkv_opt_mode not in {"hybrid", "bridge", "newton_muon", "muon"}:
    raise ValueError(
        f"QKV_OPT_MODE must be hybrid, bridge, newton_muon, or muon; got {args.qkv_opt_mode!r}"
    )
...
#   optimizer3 -> QKV hybrid, bridge, or exact Muon/Newton-Muon control
...
def _audit_eval_loss(xb: Tensor, yb: Tensor) -> float:
    ...
    # Match the gradient-capture forward as closely as possible:
    # autograd enabled, FP32 compute, same raw model and eval mode.
    # We do not call backward; the graph is discarded after extracting
    # the scalar loss.
...
def run_attention_geometry_audit(step: int) -> dict:
    ...
    # The first eval-mode call can compile a distinct graph and incur a small
    # one-time numerical offset. Warm both audit batches before recording the
    # reference losses.
    _audit_eval_loss(x_a, y_a)
    _audit_eval_loss(x_b, y_b)
