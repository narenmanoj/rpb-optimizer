...
# -----------------------------------------------------------------------------
# accum_xtx ops (NEW). Only called when precond_flag is True.

_DUMMY_SCALAR_CACHE: dict[tuple[torch.device, torch.dtype], Tensor] = {}

def _dummy_scalar_like(t: Tensor) -> Tensor:
    key = (t.device, t.dtype)
    out = _DUMMY_SCALAR_CACHE.get(key, None)
    if out is None or out.device != t.device or out.dtype != t.dtype:
        out = torch.zeros((), device=t.device, dtype=t.dtype)
        _DUMMY_SCALAR_CACHE[key] = out
    return out

@torch.compile(dynamic=False, fullgraph=True)
def _accum_xtx_impl_v3(x_2d: Tensor, accum: Tensor, count: Tensor, tmp: Tensor) -> None:
    with torch.no_grad():
        A = x_2d.transpose(0, 1)  # (d,T)
        ns_line_1(A, out=tmp)     # (d,d)
        accum.add_(tmp, alpha=1.0 / x_2d.size(0))
        count.add_(1)

@torch.compile(dynamic=False, fullgraph=True)
def _accum_xtx_blocks4_impl_v3(x_2d: Tensor, accum: Tensor, count: Tensor, tmp: Tensor) -> None:
    with torch.no_grad():
        T = x_2d.size(0)
        d = x_2d.size(1) // 4
        A = x_2d.view(T, 4, d).permute(1, 2, 0)  # (4,d,T)
        ns_line_1(A, out=tmp)  # (4,d,d)
        tmp.mul_(1.0 / T)
        accum.add_(tmp)
        count.add_(1)

@torch.library.custom_op("nanogpt::accum_xtx_v3", mutates_args=("accum", "count", "tmp"))
def accum_xtx_op_v3(x_2d: Tensor, accum: Tensor, count: Tensor, tmp: Tensor) -> Tensor:
    _accum_xtx_impl_v3(x_2d, accum, count, tmp)
    return _dummy_scalar_like(accum)

@accum_xtx_op_v3.register_fake
def _(x_2d: Tensor, accum: Tensor, count: Tensor, tmp: Tensor):
    return accum.new_empty(())

@torch.library.custom_op("nanogpt::accum_xtx_blocks4_v3", mutates_args=("accum", "count", "tmp"))
def accum_xtx_blocks4_op_v3(x_2d: Tensor, accum: Tensor, count: Tensor, tmp: Tensor) -> Tensor:
    _accum_xtx_blocks4_impl_v3(x_2d, accum, count, tmp)
    return _dummy_scalar_like(accum)

@accum_xtx_blocks4_op_v3.register_fake
def _(x_2d: Tensor, accum: Tensor, count: Tensor, tmp: Tensor):
    return accum.new_empty(())

# -----------------------------------------------------------------------------
# Muon optimizer (with preconditioner addition)
...
class Muon(torch.optim.Optimizer):
    ...
    Preconditioner ADDITION:
      - Model maintains activation XTX accumulators (persistent=False buffers).
      - Optimizer keeps an EWMA covariance per param and refreshes inv every PRECOND_EVERY steps.
      - Apply inv (left or right, depending on weight) before Newton-Schulz orthogonalization.
    ...
    def _get_eye(self, d: int, device) -> Tensor:
        key = (d, device)
        eye = self._eye_cache.get(key, None)
        if eye is None:
    ...
    def _get_eye4(self, d: int, device) -> Tensor:
        key = (d, device)
        eye4 = self._eye4_cache.get(key, None)
        if eye4 is None:
            eye4 = torch.eye(d, device=device, dtype=torch.float32).unsqueeze(0).repeat(4, 1, 1).contiguous()
    ...
    def attach_preconditioner(self, model: nn.Module):
        ...
        for block in model.blocks:
            ...
            I = self._get_eye(d, p.device)
            if ("cov" not in st) or (st["cov"].shape != (d, d)) or (st["cov"].device != p.device):
                st["cov"] = (PRECOND_INIT_DIAG * I).contiguous()
            if ("inv" not in st) or (st["inv"].shape != (d, d)) or (st["inv"].device != p.device):
                st["inv"] = I.clone()
            if ("Kbuf" not in st) or (st["Kbuf"].shape != (d, d)) or (st["Kbuf"].device != p.device):
                st["Kbuf"] = torch.empty((d, d), device=p.device, dtype=torch.float32)
            if ("precond_buf" not in st) or (st["precond_buf"].shape != p.shape) or (st["precond_buf"].device != p.device) or (st["precond_buf"].dtype != p.dtype):
                st["precond_buf"] = torch.empty_like(p)
    ...
    @torch.no_grad()
    def _chol_inv_inplace(self, Kbuf: Tensor, eye: Tensor, out_inv: Tensor) -> None:
        L, _ = torch.linalg.cholesky_ex(Kbuf, check_errors=False)
        torch.cholesky_inverse(L, upper=False, out=out_inv)

    @torch.no_grad()
    def _refresh_one(self, p: Tensor, st: dict) -> None:
        ref = self._precond_map.get(p, None)
        if ref is None:
            return
        ...
        if kind == "qkvo":
            xtx_qkv: Tensor = ref["xtx_qkv"]
            cnt_qkv: Tensor = ref["cnt_qkv"]
            xtx_o: Tensor = ref["xtx_o"]
            cnt_o: Tensor = ref["cnt_o"]

            # ---- finalize means (in-place), reset counts ----
            denom_qkv = cnt_qkv.to(torch.float32).clamp_min(1.0)
            xtx_qkv.mul_(denom_qkv.reciprocal())
            cnt_qkv.zero_()

            denom_o = cnt_o.to(torch.float32).clamp_min(1.0)
            xtx_o.mul_(denom_o.reciprocal())
            cnt_o.zero_()

            cov: Tensor = st["cov"]   # (4,d,d)  with cov[:3] identical by construction
            inv: Tensor = st["inv"]   # (4,d,d)
            Kbuf: Tensor = st["Kbuf"] # (4,d,d)

            # ---- EWMA update ----
            cov[:3].mul_(PRECOND_EWMA).add_(xtx_qkv.unsqueeze(0), alpha=(1.0 - PRECOND_EWMA))
            cov[3].mul_(PRECOND_EWMA).add_(xtx_o, alpha=(1.0 - PRECOND_EWMA))

            # ---- ridge (trace/d * const) ----
            tr = cov.diagonal(dim1=-2, dim2=-1).sum(-1)  # (4,)
            ridge = tr / float(d) * 0.2

            # ---- build only the matrices we actually invert ----
            # QKV: use slot 0
            K0 = Kbuf[0]
            K0.copy_(cov[0])
            K0.diagonal().add_(ridge[0])

            # O: use slot 3
            K3 = Kbuf[3]
            K3.copy_(cov[3])
            K3.diagonal().add_(ridge[3])

            # ---- invert ONCE for QKV, once for O ----
            eye = self._get_eye(d, p.device)

            self._chol_inv_inplace(K0, eye, inv[0])
            inv[1].copy_(inv[0])
            inv[2].copy_(inv[0])

            self._chol_inv_inplace(K3, eye, inv[3])
        ...
        if kind == "c_fc":
            ...
            eye = self._get_eye(d, p.device)
            self._chol_inv_inplace(Kbuf, eye, inv)

            xtx.zero_()
            return

    @torch.no_grad()
    def _apply_inv(self, p: Tensor, grad: Tensor, st: dict) -> Tensor:
        ref = self._precond_map.get(p, None)
        if ref is None:
            return grad
        kind = ref["kind"]
        ...
        buf: Tensor = st.get("precond_buf", None)
        if buf is None or buf.shape != grad.shape or buf.device != grad.device or buf.dtype != grad.dtype:
            buf = torch.empty_like(grad)
            st["precond_buf"] = buf
    ...
    def step(self):
        ...
        #
        # This preserves the baseline ML pipeline behavior, with only the
        # preconditioner addition (refresh/apply) inserted.
        rank = dist.get_rank()
        world_size = dist.get_world_size()

        do_refresh = self._precond_attached and (self._precond_step % PRECOND_EVERY == PRECOND_EVERY - 1)
        ...
        for group in self.param_groups:
            ...
            for base_i in range(0, len(params), world_size):
                ...
                # This gives strange dynamo warnings (baseline behavior preserved)
        ...
        for group in self.param_groups:
            ...
            for base_i in range(0, len(params), world_size):
                ...
                if base_i + rank < len(params):
                    ...
                    # momentum buffer init (NEW: can't rely on len(state)==0 because precond adds state entries)
                    if ("momentum_buffer" not in state
                        or state["momentum_buffer"].shape != grad.shape
                        or state["momentum_buffer"].device != grad.device
                        or state["momentum_buffer"].dtype != grad.dtype):
                        state["momentum_buffer"] = torch.zeros_like(grad)

                    # preconditioner refresh (NEW)
...
class CausalSelfAttention(nn.Module):
    def __init__(self, dim: int, num_heads: int, max_seq_len: int, head_dim=128):
        super().__init__()
        self.model_dim = dim  # NEW: used by optimizer binding
        ...
        # NEW: activation covariance accumulators (persistent=False)
        ...
        self.precond_xtx_count = nn.Buffer(torch.zeros((), dtype=torch.int32), persistent=False)
        ...
        self.precond_o_xtx_count = nn.Buffer(torch.zeros((), dtype=torch.int32), persistent=False)

    def forward(self, x: Tensor, ve: Tensor | None, lambdas: Tensor, block_mask: BlockMask, precond_flag: bool):
        if precond_flag:
            x2d = x.flatten(0, -2)
            torch.ops.nanogpt.accum_xtx_v3(x2d, self.precond_xtx_accum, self.precond_xtx_count, self.precond_xtx_tmp)
        ...
        if precond_flag:
            y2d = y.flatten(0, -2)
            torch.ops.nanogpt.accum_xtx_v3(y2d, self.precond_o_xtx_accum, self.precond_o_xtx_count, self.precond_o_xtx_tmp)
...
class MLP(nn.Module):
    def __init__(self, dim: int):
        ...
        self.precond_fc_xtx_count = nn.Buffer(torch.zeros((), dtype=torch.int32), persistent=False)
    ...
    def forward(self, x: Tensor, precond_flag: bool):
        if precond_flag:
            x2d = x.flatten(0, -2)
            torch.ops.nanogpt.accum_xtx_v3(x2d, self.precond_fc_xtx_accum, self.precond_fc_xtx_count, self.precond_fc_xtx_tmp)
...
class Block(nn.Module):
    ...
    def forward(self, x: Tensor, ve: Tensor | None, x0: Tensor, lambdas: Tensor, sa_lambdas: Tensor, block_mask: BlockMask, precond_flag: bool):
        x = lambdas[0] * x + lambdas[1] * x0
        if self.attn is not None:
            x = x + self.attn(norm(x), ve, sa_lambdas, block_mask, precond_flag)
...
class GPT(nn.Module):
    def __init__(self, vocab_size: int, num_layers: int, num_heads: int, model_dim: int, max_seq_len: int):
        ...
        # NOTE: keep baseline FP8 scaling (w_s=2**-9). Only preconditioner changes elsewhere.
    ...
    def forward(self, input_seq: Tensor, target_seq: Tensor, sliding_window_num_blocks: Tensor, precond_flag: bool):
        ...
        for i in range(len(self.blocks)):
            ...
            x = self.blocks[i](x, ve[i], x0, lambdas[i], sa_lambdas[i], block_masks[i], precond_flag)
            if i < n:
...
# baseline lr preserved (0.05); preconditioner is the only change
...
# NEW: attach preconditioner after compile (unwrap handled inside)
optimizer2.attach_preconditioner(model)

@torch.no_grad()
def zero_precond_buffers(m: nn.Module):
    if hasattr(m, "_orig_mod"):
        m = m._orig_mod
    for mod in m.modules():
        if hasattr(mod, "precond_xtx_accum"):
            mod.precond_xtx_accum.zero_()
        if hasattr(mod, "precond_xtx_count"):
            mod.precond_xtx_count.zero_()
        if hasattr(mod, "precond_o_xtx_accum"):
            mod.precond_o_xtx_accum.zero_()
        if hasattr(mod, "precond_o_xtx_count"):
            mod.precond_o_xtx_count.zero_()
        if hasattr(mod, "precond_fc_xtx_accum"):
            mod.precond_fc_xtx_accum.zero_()
        if hasattr(mod, "precond_fc_xtx_count"):
            mod.precond_fc_xtx_count.zero_()
        if hasattr(mod, "precond_proj_xtx_accum"):
            mod.precond_proj_xtx_accum.zero_()
        if hasattr(mod, "precond_proj_xtx_count"):
            mod.precond_proj_xtx_count.zero_()
...
for wstep in range(warmup_steps):
    ...
    precond_flag = (wstep % PRECOND_EVERY == PRECOND_EVERY - 1)
    model(inputs, targets, get_window_size_blocks(1), precond_flag).backward()
    for opt in optimizers:
        opt.step()
...
# precond buffers are persistent=False, so they are NOT restored by state_dict
zero_precond_buffers(model)
    ...
    if last_step or (args.val_loss_every > 0 and step % args.val_loss_every == 0):
        ...
        with torch.no_grad():
            for _ in range(val_steps):
                ...
                val_loss += model(inputs, targets, get_window_size_blocks(step), False)
        val_loss /= val_steps
        del val_loader
