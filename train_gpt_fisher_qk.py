...
class FisherQK(torch.optim.Optimizer):
    ...
    block retains the repository's Newton-Muon path: input-Gram inverse before
    momentum, then a separate matrix-sign map.
    """
    ...
    def step(self):
        ...
        for group in self.param_groups:
            ...
            for p in group["params"]:
                ...
                # V uses the strong Newton-Muon baseline.
                gv = g_blocks[2] @ inv
                if "v_momentum_buffer" not in state:
                    state["v_momentum_buffer"] = torch.zeros_like(gv)
                vbuf = state["v_momentum_buffer"]
                ...
                v_update = -schedule * self.v_lr * math.sqrt(float(d)) * v_shaped
...
class CausalSelfAttention(nn.Module):
    def __init__(self, config):
        ...
        if self.qkv_opt_mode not in {"hybrid", "bridge", "newton_muon", "muon", "fisher_qk"}:
    ...
    def forward(self, x, precond_flag: bool = False):
        ...
        if self.qkv_opt_mode in {"hybrid", "bridge"}:
            ...
            if precond_flag and self.qkv_opt_mode in {"newton_muon", "fisher_qk"}:
...
class Hyperparameters:
    ...
    qkv_opt_mode : str = "fisher_qk"  # fisher_qk, newton_muon, hybrid, bridge, or muon
...
if args.qkv_opt_mode not in {"hybrid", "bridge", "newton_muon", "muon", "fisher_qk"}:
    raise ValueError(
        f"QKV_OPT_MODE must be hybrid, bridge, newton_muon, muon, or fisher_qk; got {args.qkv_opt_mode!r}"
    )
...
#   optimizer2 -> all non-QKV transformer matrices
#   optimizer3 -> QKV Fisher, hybrid, bridge, or exact Muon/Newton-Muon control
...
optimizer2 = Muon(
    muon_params,
    lr=args.block_matrix_lr_mult * args.learning_rate,
    momentum=args.block_matrix_momentum,
)

# Hybrid and Newton-Muon modes retain the right preconditioner on non-QKV blocks.
# Muon mode disables it everywhere, producing a full-model Muon control.
if args.qkv_opt_mode != "muon":
    optimizer2.attach_preconditioner()

if args.qkv_opt_mode == "hybrid":
    optimizer3 = RPB(
        qkv_weights,
        lr=args.rpb_eta,
        momentum=args.rpb_momentum,
        nesterov=args.rpb_nesterov,
        h_sigma=args.rpb_h_sigma,
        ...
        rowsign_power=args.rpb_rowsign_power,
        spectral_blend=args.rpb_spectral_blend,
        spectral_steps=args.rpb_spectral_steps,
        precond_blend=args.rpb_precond_blend,
        nor_enable=bool(args.rpb_nor_enable),
        nor_beta2=args.rpb_nor_beta2,
        nor_eps=args.rpb_nor_eps,
    )
    qkv_diag_name = "rpb"
elif args.qkv_opt_mode == "fisher_qk":
    optimizer3 = FisherQK(
        qkv_weights,
        ...
        v_lr=args.fisher_v_lr,
        v_momentum=args.fisher_v_momentum,
        v_nesterov=args.fisher_v_nesterov,
...
elif args.qkv_opt_mode == "bridge":
    optimizer3 = QKVBridge(
        qkv_weights,
        lr=args.qkv_control_lr,
        momentum=args.qkv_control_momentum,
        nesterov=True,
        backend_steps=args.qkv_control_steps,
        bridge_blend=args.rpb_nm_bridge_blend,
        rowsign_power=args.rpb_rowsign_power,
        radius_blend=args.rpb_radius_blend,
        headnorm_blend=args.rpb_headnorm_blend,
        h_sigma=args.rpb_h_sigma,
        r_max=(args.rpb_r_max if args.rpb_r_max > 0 else None),
        precond_refresh_period=args.rpb_precond_refresh_period,
        precond_ewma=args.rpb_precond_ewma,
    ...
    qkv_diag_name = "bridge"
else:
    optimizer3 = QKVMatrixControl(
        qkv_weights,
        mode=args.qkv_opt_mode,
        lr=args.qkv_control_lr,
        momentum=args.qkv_control_momentum,
        nesterov=True,
        backend_steps=args.qkv_control_steps,
    ...
    qkv_diag_name = args.qkv_opt_mode

optimizers = [optimizer1, optimizer2, optimizer3]

# Diagnostics: exact controls and the bridge expose ordinary QKV gradients;
# the original hybrid drives QKV only through captured activation-space statistics.
named_params = {
    "adamw": [p for p in optimizer1.param_groups[0]["params"]],
    "matrix_blocks": muon_params,
    qkv_diag_name: qkv_weights,
}
opt_names = ["adamw", "matrix_blocks", qkv_diag_name]
...
for step in range(args.num_iterations + 1):
    ...
    if diag_step:
        ...
        diag_metrics.update(optimizer3.last_diag)   # hybrid or exact-control diagnostics
