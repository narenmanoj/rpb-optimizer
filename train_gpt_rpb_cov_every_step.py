...
class RPB(torch.optim.Optimizer):
    ...
    Gram preconditioner refresh -- mirrors the Muon right-preconditioner cadence, but
    uses an every-step EWMA of the RPB input Gram. The preconditioner is the damped
    Gram inverse (E[Z^T Z] + ridge I)^{-1}. The covariance is updated every optimizer
    step from the current batch Gram, while the damped Cholesky inverse is recomputed
    only every precond_refresh_period steps (first refresh at step
    precond_refresh_period-1, like Muon's t%32==0 schedule). The cached inverse --
    identity until the first refresh -- is reused on the steps in between. The per-step
    numerator M (with momentum) and the per-head r* are computed every step; only the
    inversion is throttled.
    ...
    and blow up the step. Seeding keeps the inverse correctly scaled from the start of
    the covariance stream.
    """
    def __init__(self, params, lr=0.5, momentum=0.95, nesterov=True, h_sigma=8.0,
        ...
        # Expensive pullback diagnostics are disabled by default. Set
        # RPB_PULLBACK_DIAG_EVERY>0 to log them periodically.
        self.pullback_diag_every = int(pullback_diag_every)
        self.global_step = 0
    ...
    @torch.no_grad()
    def step(self):
        # Refresh the Gram inverse only every precond_refresh_period steps, using
        # Muon's exact schedule: t = since+1, refresh when t % period == 0 (so the
        # first refresh is at step period-1). Unlike the first refreshed RPB version,
        # the covariance EWMA itself is updated every step from the current batch Gram.
        # global_step is set from the training loop for resume-safety; it also advances
        # internally so the schedule holds if unset.
        since = max(0, int(self.global_step) - int(self._regime_step))
        do_refresh = (((since + 1) % self.precond_refresh_period) == 0)
        did_refresh = False
        do_pullback_diag = (self.pullback_diag_every > 0 and
                            (int(self.global_step) % self.pullback_diag_every == 0))
        rstar_sum = 0.0; rstar_max = 0.0; sg_sum = 0.0; gy_sum = 0.0; nheads = 0; nlayers = 0
        real_err_sum = 0.0; real_err_max = 0.0
        damped_err_sum = 0.0; damped_err_max = 0.0
        pullback_diag_count = 0
        for group in self.param_groups:
            ...
            for p in group["params"]:
                ...
                # Gram preconditioner: EWMA covariance updated every step + cached inverse,
                ...
                # Muon). The covariance is seeded with the first observed Gram instead of
                # lerping from init_diag*I -- see the class docstring for why this single
                # deviation is required for RPB.
                ...
                if state["precond_cov_seeded"]:
                    cov.lerp_(gram, 1.0 - self.precond_ewma)       # EWMA toward every-step Gram
                else:
                    cov.copy_(gram)                                # seed on first observed Gram
                ...
                if do_pullback_diag:
                    # Cheap normal-equation proxy for the activation-target realization.
                    # If inv exactly solves the current damped Gram system, then
                    #     dW @ (gram + ridge I) ~= -lr * Mp.
                    # The undamped version dW @ gram ~= -lr * Mp is closer to the actual
                    # activation-space target on the current batch.
                    target_num = Mp * (-lr)
                    realized_num = dW @ gram
                    den = target_num.norm().clamp_min(1e-12)
                    real_err = (realized_num - target_num).norm() / den

                    ridge_now = (gram.diagonal().mean() * self.ridge_mult + self.eps_gram).clamp_min(self.eps_gram)
                    damped_realized_num = realized_num + dW * ridge_now
                    damped_err = (damped_realized_num - target_num).norm() / den

                    real_err_f = float(real_err)
                    damped_err_f = float(damped_err)
                    real_err_sum += real_err_f
                    real_err_max = max(real_err_max, real_err_f)
                    damped_err_sum += damped_err_f
                    damped_err_max = max(damped_err_max, damped_err_f)
                    pullback_diag_count += 1
        ...
        self.last_diag = {
            ...
            "rpb/cov_update_every_step": 1.0,
            "rpb/pullback_diag_count": float(pullback_diag_count),
            "rpb/pullback_realization_error": (real_err_sum / pullback_diag_count) if pullback_diag_count else 0.0,
            "rpb/pullback_realization_error_max": real_err_max,
            "rpb/pullback_damped_error": (damped_err_sum / pullback_diag_count) if pullback_diag_count else 0.0,
            "rpb/pullback_damped_error_max": damped_err_max,
        }
...
class Hyperparameters:
    ...
    rpb_pullback_diag_every : int = 0       # expensive pullback diagnostics; 0 disables
...
args.rpb_pullback_diag_every = _env_i("RPB_PULLBACK_DIAG_EVERY", args.rpb_pullback_diag_every)
...
optimizer3 = RPB(qkv_weights, lr=args.rpb_eta, momentum=args.rpb_momentum,
                 ...
                 pullback_diag_every=args.rpb_pullback_diag_every)
